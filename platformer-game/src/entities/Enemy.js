...
class Enemy extends Phaser.Physics.Arcade.Sprite {
  ...
  patrol(time) {
    ...
    const { ray, hasHit } = this.raycast(this.body, this.platformCollidersLayer, 
      {raylength: 70, precision: 1, steepness: 0.5} );

    // enemy needs time to make the turn or else the enemy will be in a infinite loop 
    // turning back and forth since the ray is not hitting the platform
  ...
  // Enemy is source of damage for player (when player hits enemy body)
  deliversHit() {}
